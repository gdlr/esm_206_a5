...
The heaviest salamanders were all found in stream channels (Fig. 3), and stream position has a significant effect on salamander size (one-way ANOVA(`r weight_list[1]`, `r weight_list[2]`) = `r round(weight_list[7], 2)`, *p* = `r round(weight_list[9], 2)`). A post-hoc Tukey test shows that salamander mean weights in side channels (mean = `r round(chan_means[3], 2)`g) were significantly different from those in pools (mean `r round(chan_means[2], 2)`g) (*p* < 0.05). Salamanders found in channels were heavier (mean = `r round(chan_means[1], 2)`g) as those found in side channels (mean = `r round(chan_means[3], 2)`g) on average, though the difference in means was not significant (*p* > 0.05). 
